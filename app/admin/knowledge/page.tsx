...
export default function KnowledgeBasePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    setStatusMessage({ type: 'loading', text: 'Sedang memproses dan menyimpan ke Supabase (via Google AI)...' });
    ...
    try {
      ...
      if (response.ok) {
        setStatusMessage({ type: 'success', text: 'Data berhasil disuntikkan ke AI Knowledge Base!' });
        setContent(''); 
      } else {
        setStatusMessage({ type: 'error', text: data.error || 'Terjadi kesalahan saat menyimpan data.' });
      }
    } catch (error) {
      console.error(error);
  ...
  const handleBulkImport = async () => {
    ...
    try {
      ...
      setBulkStatus({ type: 'error', text: 'Format JSON tidak valid. Pastikan format sesuai contoh.' });
      return;
    }
    ...
    const documents = parsed.map(item => ({
      ...
      metadata: item.metadata || (item.kategori ? { kategori: item.kategori } : {}),
    })).filter(d => d.content.trim());

    if (documents.length === 0) {
      setBulkStatus({ type: 'error', text: 'Tidak ada dokumen valid ditemukan dalam JSON.' });
      return;
    }

    setIsBulkLoading(true);
    setBulkProgress({ current: 0, total: documents.length });
    setBulkStatus({ type: 'loading', text: `Memproses ${documents.length} dokumen...` });
    ...
    try {
      ...
      if (response.ok) {
        ...
        setBulkStatus({ type: 'error', text: data.error || 'Terjadi kesalahan.' });
      }
    } catch (error) {
      console.error(error);
    ...
    <div className="space-y-8 w-full max-w-5xl">
      <div>
        ...
        <p className="text-white-100/70 text-sm">
          Tambahkan informasi baru mengenai diri Anda, project, atau tech stack agar AI Digital Twin dapat mempelajarinya.
        </p>
      </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  <div>
                    ...
                    >
                      ...
                      <option value="pendidikan">Pendidikan dan Sertifikasi</option>
                  ...
                  {statusMessage.text && (
                    <div className={`p-4 mt-4 rounded-xl text-sm ${
                      ...
                      <p className="flex items-center gap-2">
                          <span className={`w-2 h-2 rounded-full ${statusMessage.type === 'success' ? 'bg-green-400' : 'bg-red-400'}`}></span>
                          {statusMessage.text}
                      </p>
                    </div>
                  )}
            ...
            {activeTab === 'bulk' && (
              <>
                ...
                <p className="text-sm text-white-100/50 mb-4">
                  Paste JSON array berisi semua dokumen knowledge sekaligus. Setiap item akan di-embed dan disimpan otomatis.
                </p>
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-black-100/50 border border-white-100/10 rounded-xl p-4">
                    <p className="text-xs font-medium text-white-100/60 mb-2">📋 Format yang didukung:</p>
                    <pre className="text-xs text-cyan/80 font-mono overflow-x-auto whitespace-pre-wrap">{sampleJson}</pre>
                  </div>
                  ...
                  {bulkStatus.text && (
                    <div className={`p-4 rounded-xl text-sm ${
                      bulkStatus.type === 'success' ? 'bg-green-500/10 text-green-400 border border-green-500/20' :
                      bulkStatus.type === 'warning' ? 'bg-yellow-500/10 text-yellow-400 border border-yellow-500/20' :
                      bulkStatus.type === 'error' ? 'bg-red-500/10 text-red-400 border border-red-500/20' :
                      'bg-blue-500/10 text-blue-400 border border-blue-500/20'
                    }`}>
                      <p className="flex items-center gap-2">
                        <span className={`w-2 h-2 rounded-full ${
                          bulkStatus.type === 'success' ? 'bg-green-400' :
                          bulkStatus.type === 'warning' ? 'bg-yellow-400 animate-pulse' :
                          bulkStatus.type === 'loading' ? 'bg-blue-400 animate-pulse' :
                          'bg-red-400'
                        }`}></span>
                        {bulkStatus.text}
                      </p>
        ...
        <div className="space-y-6">
          <div className="bg-black-200 border border-white-100/10 rounded-2xl p-6 relative overflow-hidden">
            ...
            <p className="text-sm text-white-100/60 leading-relaxed mb-4">
              Setiap teks yang Anda masukkan akan dikirim ke API <strong className="text-white-100/90">Google AI</strong> untuk diubah menjadi kumpulan angka matematis berdimensi tinggi (<i>Vector Embedding</i>).
            </p>
            <p className="text-sm text-white-100/60 leading-relaxed">
              Vektor tersebut kemudian disimpan di database <strong className="text-white-100/90">Supabase (pgvector)</strong>. Ketika user bertanya di chatbot, AI akan mencari teks di database Anda yang secara makna paling mirip dengan pertanyaan tersebut!
            </p>
          </div>
          ...
          <div className="bg-gradient-to-br from-yellow-500/10 to-yellow-500/5 border border-yellow-500/20 rounded-2xl p-6">
            ...
            <ul className="text-xs text-yellow-500/80 space-y-2.5 list-disc list-inside">
              ...
              <li>Sertakan tanggal kejadian atau periode waktu jika relevan.</li>
