...
export const Navbar = ({ children, className }: NavbarProps) => {
  ...
  return (
    <motion.div
      ...
      className={cn("fixed inset-x-0 top-2 z-[5000] w-full", className)}
    >
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          exit={{ opacity: 0 }}
          className={cn(
            "absolute inset-x-0 top-16 z-50 flex w-full flex-col items-start justify-start gap-4 rounded-lg bg-black-200 px-4 py-8 shadow-[0_0_24px_rgba(0,0,0,0.3)] border border-white/[0.1]",
            className,
          )}
        >
          {children}
